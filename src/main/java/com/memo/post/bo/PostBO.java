...
public class PostBO {
	...
	public int updatePost(String userLoginId, int userId,
		...
		if (file != null) {
			try {
				// 1. 서버에 이미지를 업로드 하고 imagePath를 받아온다.
				imagePath = fileManagerService.saveFile(userLoginId, file);
				
				// 2. 기존에 이미지가 있었으면 파일을 제거한다.
				// -- 1번에서 업로드가 실패 할 수 있으므로 성공한 후 제거
	...
	public int deletePost(int postId, int userId) {
		...
		Post post = getPostById(postId);
		if (post == null) {
			logger.warn("[delete post] 삭제할 메모가 존재하지 않음. postId:{}", postId);
			return 0;
		}
		...
		if (post.getImagePath() != null) {
			try {
				...
				logger.error("[delete post] 파일 삭제 중 이미지 파일 삭제 실패. postId:{}", postId);
			}
		}
